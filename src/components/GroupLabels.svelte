...
<script>
   ...
   */
  const left = $derived(values => $xScale(max(values, $x)) / Math.max(...$xRange))
  const top = $derived(values => $yScale(max(values, $y)) / Math.max(...$yRange))
</script>
  ...
  <div
    ...
    style='
      top:{top(group.values) * 100}%;
        right:{(1.0 - left(group.values)) * 100}%;
    '
  >
    {cap($z(group))}
  </div>
...
<style>
  .label {
    position: absolute;
    transform: translate(-30%, -100%) translateY(1px);
    font-size: 13px;
  }
</style>
